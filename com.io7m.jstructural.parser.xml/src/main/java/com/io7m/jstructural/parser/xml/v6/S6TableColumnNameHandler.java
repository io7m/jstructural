...
final class S6TableColumnNameHandler extends S6ElementHandler
{
  private final STableColumnName.Builder<SParsed> name_builder;

  S6TableColumnNameHandler(
    final S6ElementHandler in_parent,
    final Attributes in_attributes,
    final Locator2 in_locator)
  {
    super(in_parent, in_locator);
    this.name_builder = STableColumnName.builder();
    this.name_builder.setData(PARSED);
    this.name_builder.setLexical(this.lexical());

    final Map<String, String> am = S6Attributes.attributeMap(in_attributes);
    if (am.containsKey("type")) {
      this.name_builder.setType(
    ...
    this.name_builder.setName(am.get("name"));
  }
  ...
  @Override
  STableColumnName<SParsed> finishContent()
  {
    return this.name_builder.build();
  }
}
