...
final class S6SectionHandler extends S6ElementHandler
{
    ...
    final Locator2 in_locator)
  {
    ...
    this.type = Optional.ofNullable(am.get("type")).map(text -> STypeName.of(PARSED, text));
    this.id = Optional.ofNullable(am.get("id")).map(text -> SBlockID.of(PARSED, text));
  ...
  SSectionType<SParsed> finishContent()
  {
    if (this.content.isEmpty()) {
      return SSectionWithSections.of(PARSED, this.type, this.id, this.title, Vector.empty());
    }
    ...
    switch (first.blockKind()) {
      case BLOCK_SUBSECTION_CONTENT: {
        ...
        return SSectionWithSubsectionContent.of(PARSED, this.type, this.id, this.title, cc);
      }

      case BLOCK_SUBSECTION: {
        final Vector<SSubsectionType<SParsed>> cc =
          Vector.ofAll(this.content).map(c -> (SSubsectionType<SParsed>) c);
        return SSectionWithSubsections.of(PARSED, this.type, this.id, this.title, cc);
      }

      case BLOCK_SECTION: {
        final Vector<SSectionType<SParsed>> cc =
          Vector.ofAll(this.content).map(c -> (SSectionType<SParsed>) c);
        return SSectionWithSections.of(PARSED, this.type, this.id, this.title, cc);
      }
