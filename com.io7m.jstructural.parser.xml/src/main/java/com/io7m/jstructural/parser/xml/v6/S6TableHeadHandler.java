...
final class S6TableHeadHandler extends S6ElementHandler
{
  private final STableHeader.Builder<SParsed> header_builder;
    ...
    final Locator2 in_locator)
  {
    ...
    this.header_builder = STableHeader.builder();
    this.header_builder.setData(PARSED);
    this.header_builder.setLexical(this.lexical());

    final Map<String, String> am = S6Attributes.attributeMap(in_attributes);
    if (am.containsKey("type")) {
      this.header_builder.setType(
    ...
    final SModelType<SParsed> c)
  {
    Preconditions.checkPrecondition(
      c,
      c instanceof STableColumnName,
      x -> "Content must be table column name");

    if (c instanceof STableColumnName) {
      this.header_builder.addNames((STableColumnName<SParsed>) c);
    }
  }
  ...
  @Override
  STableHeader<SParsed> finishContent()
  {
    return this.header_builder.build();
  }
}
