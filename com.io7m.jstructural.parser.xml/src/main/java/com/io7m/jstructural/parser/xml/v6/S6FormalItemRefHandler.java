...
final class S6FormalItemRefHandler extends S6ElementHandler
{
  private final SFormalItemReference.Builder<SParsed> formal_builder;

  S6FormalItemRefHandler(
    final S6ElementHandler in_parent,
    final Attributes in_attributes,
    final Locator2 in_locator)
  {
    super(in_parent, in_locator);
    this.formal_builder = SFormalItemReference.builder();
    this.formal_builder.setData(PARSED);
    this.formal_builder.setLexical(this.lexical());

    final Map<String, String> am = S6Attributes.attributeMap(in_attributes);
    if (am.containsKey("type")) {
      this.formal_builder.setType(
    ...
    this.formal_builder.setTarget(SBlockID.of(PARSED, am.get("target")));
  }
  ...
  {
    return this.formal_builder.build();
  }
}
